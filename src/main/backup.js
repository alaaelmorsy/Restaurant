...
const { getPool, DB_NAME, getConfig } = require('../db/connection');
...
async function createDbDumpGz(){
  const cfg = getConfig();
  // Prefer mysqldump for speed and correctness
  const sql = await dumpWithMysqldump(cfg);
...
function registerBackupIPC(){
  ...
  ipcMain.handle('backup:save_db_to_file', async (_e, payload) => {
    try{
      const savePath = payload && payload.savePath;
      if(!savePath){ 
        return { ok:false, error: 'لم يتم تحديد مسار الحفظ' };
      }
      
      const sql = await createDbDumpSql();
      fs.writeFileSync(savePath, sql);
...
module.exports = { registerBackupIPC, emailDbBackup };