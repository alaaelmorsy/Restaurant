...
class LocalZatcaBridge {
  ...
  async submitSaleById(sale_id){
    ...
    try{
      ...
      try{
        ...
        // Check for NOT_REPORTED or rejection in response
        try{
          const pool = await getPool();
          const conn = await pool.getConnection();
          try{
  ...
  async buildBodyFromSaleId(saleId) {
    ...
    try {
      ...
      const vatTotal = Number(sale.vat_total || 0);
      const grandTotal = Number(sale.grand_total || (totalWithoutVAT + vatTotal));
      const rateBase = totalWithoutVAT > 0 ? totalWithoutVAT : subTotal;
      const rawRate = rateBase > 0 ? (vatTotal / rateBase) * 100 : (Number(zcfg?.taxRate) || 15);
      const taxRate = this.normalizeVatRate(rawRate);
      ...
      const sumPreVat = items.reduce((acc, it) => {
        ...
        const qtyNorm = isCredit ? Math.abs(qtyRaw) : qtyRaw;
        const priceNorm = isCredit ? Math.abs(priceRaw) : priceRaw;
        return acc + (priceNorm * qtyNorm);
      }, 0);
      const discountForSplit = isCredit ? Math.abs(discount) : discount;
      for (const it of items) {
        const qtyRaw = Number(it.qty || it.quantity || 1);
        const unitPriceRaw = Number(it.price || 0); // assumed pre-VAT
        ...
        const lineDiscount = Number((discountForSplit * share).toFixed(2));
