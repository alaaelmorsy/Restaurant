...
class ZatcaSalesIntegration {
    ...
    async convertSaleToZatcaFormat(saleData, zatcaConfig) {
        ...
        // جلب إعدادات الضريبة للتحقق من طريقة حساب الخصم
        let includeVAT = true;
        let vatRate = 15;
        try {
            ...
            try {
                ...
                if (rows && rows[0]) {
                    ...
                    vatRate = Number(rows[0].vat_percent || 15);
                }
            } finally { conn.release(); }
        } catch (_) {}

        vatRate = this.normalizeVatRate(vatRate);

        // حساب المجاميع
        const subtotal = Math.abs(parseFloat(saleData.subtotal || 0));
        let discount = Math.abs(parseFloat(saleData.discount_amount || 0));
        const vatTotal = Math.abs(parseFloat(saleData.vat_amount || 0));
        const totalWithVAT = Math.abs(parseFloat(saleData.total || 0));

        // إذا كانت الأسعار شاملة الضريبة، فإن الخصم المخزن عادة ما يكون شاملاً الضريبة أيضاً
        // نحتاج لتحويل الخصم إلى صافي لكي يظهر بشكل صحيح في الفاتورة (Allowance Amount is Tax Exclusive)
        if (includeVAT && discount > 0) {
            discount = Number((discount / (1 + (vatRate / 100))).toFixed(2));
        }
        ...
        const invoiceData = {
            ...
            totals: {
                subtotal: subtotal,
                discount: discount, // أصبح الآن صافياً
                vatTotal: vatTotal,
    ...
    async convertSaleItemsToZatcaFormat(items) {
        ...
        defaultVat = this.normalizeVatRate(defaultVat);
        ...
        return items.map(item => {
            ...
            const vatRate = this.normalizeVatRate(num(item.vat_rate ?? defaultVat));
    ...
    mapExternalPayloadToInvoiceData(external) {
        ...
        const items = Array.isArray(external.products) ? external.products.map(p => ({
            ...
            unitPrice: num(p.selling_price ?? p.price ?? 0),
            lineTotal: num(p.total_selling_after_dis ?? p.total_net ?? (num(p.selling_price ?? p.price ?? 0) * num(p.count || 1))),
            vatRate: this.normalizeVatRate(num(p.tax_rate ?? external.tax_rate ?? 15)),
            taxCategory: 'S',
            unitCode: 'PCE'
        })) : [];
