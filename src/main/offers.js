...
async function ensureTables(conn){
  ...
  // Ensure excluded_categories column exists for global offers (JSON array of category names)
  try{ await conn.query('ALTER TABLE offers ADD COLUMN excluded_categories TEXT NULL'); }catch(_){ }
...
function registerOffersIPC(){
  ...
  ipcMain.handle('offers:list', async (_e, q) => {
    if(isSecondaryDevice()){ return await fetchFromAPI('/offers', q); }
    const query = q || {};
    const params = [];
    const where = [];
  ...
  ipcMain.handle('offers:get', async (_e, idObj) => {
    if(isSecondaryDevice()){ return await fetchFromAPI(`/offers/${idObj}`, {}); }
    ...
    try{
      ...
      try{
        await conn.query(`USE \`${DB_NAME}\``);
        await ensureTables(conn);
        const [rows] = await conn.query('SELECT * FROM offers WHERE id = ? LIMIT 1', [id]);
        if(!rows || !rows.length){ return { ok:false, error:'العرض غير موجود' }; }
        const offer = rows[0];
        if(offer.is_global == null){ offer.is_global = 0; }
        return { ok:true, item: offer };
      } finally { conn.release(); }
    }catch(e){ console.error(e); return { ok:false, error:'تعذر تحميل العرض' }; }
  });
  ...
  ipcMain.handle('offers:add', async (_e, payload) => {
    ...
    const excluded_categories = payload && payload.excluded_categories ? String(payload.excluded_categories) : null;
    try{
      const pool = await getPool(); const conn = await pool.getConnection();
      try{
        await conn.query(`USE \`${DB_NAME}\``);
        ...
        const [res] = await conn.query(
          'INSERT INTO offers(name, description, mode, value, start_date, end_date, is_active, is_global, excluded_categories) VALUES (?,?,?,?,?,?,?,?,?)',
          [name, description, mode, value, start_date, end_date, is_active, Number(payload && payload.is_global ? 1 : 0), excluded_categories]
        );
  ...
  ipcMain.handle('offers:update', async (_e, idObj, payload) => {
    ...
    try{
      ...
      try{
        ...
        await conn.query(
          'UPDATE offers SET name=?, description=?, mode=?, value=?, start_date=?, end_date=?, is_active=?, is_global=?, excluded_categories=? WHERE id=?',
          [name, description, mode, value, start_date, end_date, is_active, Number(payload && payload.is_global ? 1 : 0), excluded_categories, id]
        );
        return { ok:true };
      } finally { conn.release(); }
    ...
    try{
      ...
      try{
        ...
        const [rows] = await conn.query(`
          ...
          ORDER BY o.is_global DESC, o.id DESC
          LIMIT 1
  ...
  ipcMain.handle('offers:get_products', async (_e, idObj) => {
    const id = (idObj && idObj.id) ? idObj.id : idObj;
    if(isSecondaryDevice()){ return await fetchFromAPI(`/offers/${id}/products`); }
    if(!id) return { ok:false, error:'معرّف العرض مفقود' };
    try{
      const pool = await getPool(); const conn = await pool.getConnection();
      try{
...
(async () => {
  try{
    const pool = await getPool(); const conn = await pool.getConnection();
    try{ await conn.query(`USE \`${DB_NAME}\``); await ensureTables(conn); } finally { conn.release(); }
  }catch(e){ console.error('offers:init ensure tables failed', e); }
})();
