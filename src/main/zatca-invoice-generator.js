...
class ZatcaInvoiceGenerator {
    ...
    async generateInvoiceXML(invoiceData) {
        try {
            ...
            const standardVatRate = this.getStandardVatRate(invoiceData);
            ...
            const invoiceXML = {
                Invoice: {
                    ...
                    ...(function(ctx){
                        ...
                        const vatOnDiscount = (docDiscount * (standardVatRate / 100));
                        const adjustedVatTotal = Number((sumVat - vatOnDiscount).toFixed(2));
                        ...
                        const totalsRecalc = {
                            ...
                            totalWithVAT: Number((sumLineNet - docDiscount + docCharges + adjustedVatTotal).toFixed(2))
                        };
                        ...
                        if (docDiscount > 0) {
                            allowanceCharges.push({
                                ...
                                'cbc:Amount': { '@_currencyID': 'SAR', '#text': docDiscount.toFixed(2) },
                                'cac:TaxCategory': {
                                    'cbc:ID': 'S',
                                    'cbc:Percent': standardVatRate.toString(),
                        ...
                        if (docCharges > 0) {
                            allowanceCharges.push({
                                ...
                                'cbc:AllowanceChargeReason': 'Extra Charge',
                                'cbc:Amount': { '@_currencyID': 'SAR', '#text': docCharges.toFixed(2) },
                                'cac:TaxCategory': {
                                    'cbc:ID': 'S',
                                    'cbc:Percent': standardVatRate.toString(),
                                    'cac:TaxScheme': { 'cbc:ID': 'VAT' }
                        ...
                        return {
                            ...
                            'cac:TaxTotal': ctx.buildTaxTotal(totalsRecalc, invoiceData.taxBreakdown, standardVatRate),
    ...
    normalizeVatRate(rate, category) {
        const parsed = Math.abs(Number(rate ?? 0));
        const normalized = parsed > 0 && parsed < 1 ? Number((parsed * 100).toFixed(2)) : parsed;
        if ((category || 'S') === 'S') {
            return normalized === 5 || normalized === 15 ? normalized : 15;
        }
        return normalized;
    }

    getStandardVatRate(data) {
        const items = data?.items || [];
        for (const item of items) {
            const category = item.taxCategory || 'S';
            if (category === 'S') {
                return this.normalizeVatRate(item.vatRate ?? 15, category);
            }
        }
        const breakdown = data?.taxBreakdown || [];
        for (const tax of breakdown) {
            const category = tax.categoryCode || 'S';
            if (category === 'S') {
                return this.normalizeVatRate(tax.rate ?? 15, category);
            }
        }
        return 15;
    }
     ...
     */
    buildTaxTotal(totals, taxBreakdown, standardVatRate) {
        const taxSubtotals = [];
        ...
        if (taxBreakdown && Array.isArray(taxBreakdown) && taxBreakdown.length) {
            for (const tax of taxBreakdown) {
                ...
                const categoryCode = tax.categoryCode || 'S';
                const tRate = this.normalizeVatRate(tax.rate ?? 15, categoryCode);
                const tRateStr = tRate === 15 || tRate === 5 ? tRate.toString() : tRate.toFixed(2);
                taxSubtotals.push({
                    'cbc:TaxableAmount': {
                        '@_currencyID': 'SAR',
            ...
            const standardRate = this.normalizeVatRate(standardVatRate ?? 15, 'S');
            const standardRateStr = standardRate === 15 || standardRate === 5 ? standardRate.toString() : standardRate.toFixed(2);
            taxSubtotals.push({
                'cbc:TaxableAmount': {
                    '@_currencyID': 'SAR',
    ...
    buildInvoiceLine(item, lineNumber) {
        ...
        const vatRate = this.normalizeVatRate(item.vatRate ?? 15, taxCategory);
        const vatRateStr = vatRate === 15 || vatRate === 5 ? vatRate.toString() : vatRate.toFixed(2);
    ...
    async generateCreditNoteXML(creditData) {
        ...
        const standardVatRate = this.getStandardVatRate(data);
        ...
        const creditNote = {
            CreditNote: {
                ...
                // Document Level Allowance (Fix for BR-KSA-84)
                ...(discount > 0 ? {
                    'cac:AllowanceCharge': [{
                        'cbc:ChargeIndicator': false,
                        'cbc:AllowanceChargeReason': 'Discount',
                        'cbc:Amount': { '@_currencyID': currency, '#text': discount.toFixed(2) },
                        'cac:TaxCategory': {
                            'cbc:ID': 'S',
                            'cbc:Percent': standardVatRate.toString(),
                            'cac:TaxScheme': { 'cbc:ID': 'VAT' }
                ...
                // Tax total (positive)
                'cac:TaxTotal': this.buildTaxTotal({ subtotal, discount, vatTotal }, data.taxBreakdown, standardVatRate),
